      ...
      <div className="bg-white dark:bg-gray-800 w-96 h-full shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="overflow-y-auto h-full pb-20">
          {notifications.map(notif => {
            ...
            return (
              <div key={notif.id} className="p-4 border-b border-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 cursor-pointer">
                <div className="flex items-start gap-3">
                  <div className="p-2 bg-blue-100 dark:bg-blue-900 rounded-lg">
                    <Icon className="w-4 h-4 text-accent-dark dark:text-blue-700" />
                  </div>
                  <div className="flex-1">
                    <p className="text-sm text-gray-900 dark:text-white">{notif.message}</p>
                    <p className="text-xs text-gray-600 dark:text-gray-300 mt-1">{notif.time}</p>
                  </div>
                </div>
              </div>
            );
