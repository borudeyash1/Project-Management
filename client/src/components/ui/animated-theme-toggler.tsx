...
export const AnimatedThemeToggler: React.FC<AnimatedThemeTogglerProps> = ({ 
  ...
  return (
    <button
      onClick={toggleTheme}
      className={`relative p-2 rounded-lg border border-border dark:border-gray-700 hover:bg-slate-50 dark:hover:bg-gray-700 transition-colors ${className}`}
    ...
    >
      <div className="relative w-5 h-5">
        ...
        {/* Moon Icon */}
        <Moon
          className={`absolute inset-0 w-5 h-5 text-blue-500 transition-all duration-${duration} ${
            isDark
              ? 'opacity-100 rotate-0 scale-100'
              : 'opacity-0 -rotate-90 scale-0'
          }`}
