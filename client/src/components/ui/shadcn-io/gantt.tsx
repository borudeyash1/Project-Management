...
export const GanttFeatureItemCard: FC<GanttFeatureItemCardProps> = ({
  ...
  return (
    <Card 
      className="h-full w-full rounded-md p-2 text-xs shadow-sm border-0 transition-all hover:shadow-md"
      style={{
    ...
    >
      <div
        className={cn(
          'flex h-full w-full items-center justify-between gap-2 text-left text-white',
      ...
      >
        ...
        >
          <div className="relative">
            ...
            {dragging && (
              <div className="pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                <div className="whitespace-nowrap rounded-lg border border-border/50 bg-background/95 px-3 py-1.5 text-foreground text-xs backdrop-blur-lg shadow-lg">
                  <span className="font-medium">{format(startAt, 'MMM dd')}</span>
                  {endAt && (
                    <>
                      <span className="mx-1.5 text-muted-foreground">→</span>
                      <span className="font-medium">{format(endAt, 'MMM dd, yyyy')}</span>
                    </>
                  )}
                </div>
              </div>
